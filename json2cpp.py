...
class JsonToCppConverter:
    def __init__(self, json_schema, header_file, implementation_file):
        ...
        self.metadata, self.classes = self.read_json_schema()
        self.namespace = self.metadata.get("namespace", "")
        self.all_classes = set(class_spec['name'] for class_spec in self.classes if class_spec.get('type') != 'enum')
        self.enums = set(enum_spec['name'] for enum_spec in self.classes if enum_spec.get('type') == 'enum')
    ...
    def generate_enum_declaration(self, enum_spec):
        ...
        cpp_code += "} // namespace " + self.namespace + "\n\n"
